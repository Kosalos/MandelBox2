...
class ViewController: UIViewController, WGDelegate {
    ...
    func initializeWidgetGroup() {
        ...
        wg.addLine()
        wg.addFloat3Dual(&control.sphere, 0,2,0.1, "Sphere",.refresh)
        wg.addSingleFloat(&control.sphereMult,  0.1,6.0,0.1, "S Mult",.refresh)
        wg.addFloat3Dual(&control.box, 0,2,0.1, "Box",.refresh)
        wg.addLine()
        wg.addToggle(tJulia)
